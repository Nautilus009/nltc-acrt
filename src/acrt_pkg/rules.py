...
def _finding(rule, target, element_name, line, path, snippet):
    return Finding(
...
class RuleEngine:
    ...
    def _run_regex(self, rule, context, target, element_name, file_stem):
        ...
        for node in self._command_nodes(context):
            if pattern.search(node.normalized_text):
                findings.append(_finding(rule, target, element_name, node.start_line, node.path(), node.normalized_text))
        return findings
        ...
        for var in context.by_kind.get("VARIABLE", []):
            ...
            if pattern.search(value):
                if var.start_line in seen_lines:
                    continue
                findings.append(_finding(rule, target, element_name, var.start_line, var.path(), f"{var.name} VALUE {value}"))
    ...
    def _run_required_divisions(self, rule, context, target, element_name, file_stem):
        ...
        return [_finding(rule, target, element_name, line, "PROGRAM", f"Missing DIVISION(s): {', '.join(missing)}")]
    ...
    def _run_required_sections(self, rule, context, target, element_name, file_stem):
        ...
        return [_finding(rule, target, element_name, line, "PROCEDURE", f"Missing SECTION(s): {', '.join(missing)}")]
    ...
    def _run_unique_paragraphs(self, rule, context, target, element_name, file_stem):
        ...
        for node in context.by_kind["SECTION"] + context.by_kind["PARAGRAPH"]:
            name = node.name
            if name in seen:
                findings.append(_finding(rule, target, element_name, node.start_line, node.path(), f"Duplicate name: {name}"))
            else:
                seen[name] = node
        return findings
    ...
    def _run_program_id_match(self, rule, context, target, element_name, file_stem):
        ...
        if not program_id:
            return [_finding(rule, target, element_name, None, "IDENTIFICATION", "PROGRAM-ID not found")]
        if program_id.upper() != file_stem.upper():
            ...
            return [_finding(rule, target, element_name, line, path, f"PROGRAM-ID {program_id}")]
        return []
    ...
    def _run_unused_sections(self, rule, context, target, element_name, file_stem):
        ...
        for node in context.by_kind["COMMAND"]:
            ...
            if "PERFORM" in node.keywords:
                ...
                if m:
                    ...
                    if ref_target in para_to_section:
                        referenced.add(para_to_section[ref_target])
        ...
        for section in sections:
            ...
            if section.name not in referenced:
                findings.append(_finding(rule, target, element_name, section.start_line, section.path(), f"Unused SECTION {section.name}"))
        return findings
    ...
    def _run_max_sections(self, rule, context, target, element_name, file_stem):
        ...
        proc_div = next((d for d in context.by_kind["DIVISION"] if d.name == "PROCEDURE"), None)
        line = proc_div.start_line if proc_div else (sections[0].start_line if sections else 1)
        return [_finding(rule, target, element_name, line, "PROCEDURE", f"SECTION count={len(sections)} (max {max_value})")]
            ...
            for cmd in para.children:
                ...
                if "PERFORM" in cmd.keywords:
                    ...
                    if m and m.group(1) == para.name:
                        findings.append(_finding(rule, target, element_name, cmd.start_line, cmd.path(), cmd.normalized_text))
